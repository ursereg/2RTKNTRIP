...
Debug Mode: {config.settings.development.debug_mode!s:<9} Max Connections: {config.settings.network.max_connections:<8}
...
def check_environment() -> None:
    ...
    required_dirs = [Path(config.settings.database.path).parent, Path(config.settings.logging.log_dir)]
...
class ServiceManager:
    ...
    def __init__(self) -> None:
        ...
        self.web_manager: Any | None = None
        self.ntrip_caster: NTRIPCaster | None = None
    ...
    def _start_web_interface(self) -> None:
        ...
        self.web_manager = create_web_manager(self.db_manager, forwarder.get_forwarder(), self.start_time)
    ...
    def _signal_handler(self, signum: int, frame: Any) -> None:
        ...
        if self.stopping:
            logger.log_system_event(
                f"Received signal {signum}, but service is already closing. Ignoring duplicate signal."
            )
            return
...
server: ServiceManager | None = None


def get_server_instance() -> ServiceManager | None:
    """Get server instance"""
    return server

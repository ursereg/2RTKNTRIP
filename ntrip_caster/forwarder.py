...
class SimpleDataForwarder:
    ...
    def force_disconnect_mount(self, mount_name):
        ...
        logger.log_info(f"Force disconnect for mount {mount_name} complete, {disconnected_count} user connections closed")
        return True
            ...
            if mount_name in self.subscribers:
                ...
                for i, subscriber in enumerate(self.subscribers[mount_name]):
                    try:
                        ...
                        logger.log_error(f"Failed to send data to subscriber #{i+1} for mount {mount_name}: {e}", 'ntrip')

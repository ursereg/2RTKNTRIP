...
def test_api_login(client: FlaskClient) -> None:
...
def test_api_add_user(client: FlaskClient) -> None:
    # Login first
    client.post(
        "/api/login", json={"username": config.settings.admin.username, "password": config.settings.admin.password}
    )
    ...
    # Add user
    response = client.post("/api/users", json={"username": "newuser", "password": "newpassword"})
...
def test_api_add_mount(client: FlaskClient) -> None:
    # Login first
    client.post(
        "/api/login", json={"username": config.settings.admin.username, "password": config.settings.admin.password}
    )
    ...
    # Add mount
    response = client.post("/api/mounts", json={"mount": "NEWMONT", "password": "mountpass"})
    assert response.status_code == 201
    assert "added successfully" in response.get_json()["message"]
